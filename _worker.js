...
async function handleLoginGetRequest(request) {
  ...
  if (userName && password) {
    const storedPasswords = await KV.get('users');
    if (storedPasswords) {
      const users = JSON.parse(storedPasswords);
      const hashedPassword = users[userName];
      if (hashedPassword && (await hashPassword(password)) === hashedPassword) {
        return await handleLogin(userName, accountNumber, 'do not need Turnstle', '');
      }
    }
    return generateLoginResponse('Incorrect username or password.');
  } else {
